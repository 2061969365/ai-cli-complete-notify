...
# 釘釘自訂機器人請將安全關鍵字設為「AI提醒」，所有 Webhook 測試與實際訊息都會包含此標識
# 可選：依來源覆寫。設定後，該來源只會傳送到自己的 URL
...
- 所有 Webhook 測試與實際訊息統一加入可見標識 `AI提醒`，測試預設內容同步包含此標識，釘釘自訂機器人可統一使用「AI提醒」作為安全關鍵字。
- 修復飛書測試卡片正文為空：未使用 AI 摘要時顯示 `AI提醒 原文` 與任務或輸出內容，摘要成功時顯示 `AI提醒 AI摘要`，並保留原有附加原文選項。
- Webhook 失敗時會在測試面板顯示平台與回傳原因，同時不暴露 Webhook URL 或 Token。
...
- 修復 Claude Watch 在 `tool_use` / `tool_result` 流程中的錯誤完成提醒：工具結果不再被當作真人新輸入重設輪次，工具呼叫與無文字 Assistant 記錄也不會啟動或殘留完成計時，只有最終的非空 Assistant 文字才會觸發完成提醒。
...
- 修復 [Issue #24](https://github.com/ZekerTop/ai-cli-complete-notify/issues/24)：Claude 來源新增預設開啟的「僅提醒互動式工作階段」選項，過濾 Agent Team、Workflow、worktree、背景 Agent 與 `claude -p` 等 SDK 衍生工作階段造成的錯誤完成提醒。
- Claude Stop Hook 與 Claude Watch 共用受限的 transcript 來源解析器，透過 `entrypoint: "sdk-cli"` / `promptSource: "sdk"` 識別 SDK 工作階段，並保留 `entrypoint: "cli"` / `promptSource: "typed"` 互動式工作階段的正常提醒。
- transcript 遺失、無法讀取或來源未知時仍沿用原本提醒流程，避免誤判；需要 `claude -p` 完成提醒時可關閉此選項。
- 保留原有 Claude 最終回覆擷取、失敗提醒、Hook 延遲、Watch 備援、摘要、通道與去重邏輯，並加入 Claude 雙路徑及既有 Gemini 修復的回歸測試。
- 「關於專案」新增 GitHub Release 更新檢查：進入頁面時自動檢查，也可手動重新檢查；顯示目前版本與最新公開版本，Windows 與 macOS 均前往 GitHub Releases 自行選擇安裝套件。
...
感謝每一位透過程式碼、文件、回饋和建議參與專案建設的貢獻者。

<a href="https://github.com/ZekerTop/ai-cli-complete-notify/graphs/contributors">
 <img alt="貢獻者" src="https://contrib.rocks/image?repo=ZekerTop/ai-cli-complete-notify&max=100&columns=12&anon=0" />
</a>
